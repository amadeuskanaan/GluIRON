    ...
    def get_niftis(population, afs_dir, workspace_dir):
        for subject in population:
            ...
            if not os.path.isfile(os.path.join(workspace_dir, subject, 'ANATOMICAL', 'MP2RAGE_UNI.nii')):
                ...
                os.system(
                    'isisconv -in %s -out %s/%s_S{sequenceNumber}_{sequenceDescription}_{echoTime}.nii -rf dcm -wdialect fsl'
                    % (anat_dir, anat_dir, subject))

                for file in os.listdir(anat_dir):
                    if 'mp2rage_p3_602B_INV1_2.98' in file:
                        os.rename(str(os.path.join(anat_dir, file)),
                                  str(os.path.join(workspace_dir, subject, 'ANATOMICAL', 'MP2RAGE_INV1_.nii')))
                    elif 'mp2rage_p3_602B_INV2_2.98' in file:
                        os.rename(str(os.path.join(anat_dir, file)),
                                  str(os.path.join(workspace_dir, subject, 'ANATOMICAL', 'MP2RAGE_INV2_.nii')))
                    elif 'mp2rage_p3_602B_DIV_Images_2.98' in file:
                        os.rename(str(os.path.join(anat_dir, file)),
                                  str(os.path.join(workspace_dir, subject, 'ANATOMICAL', 'MP2RAGE_DIV_.nii')))
                    elif 'mp2rage_p3_602B_T1_Images_2.98' in file:
                        os.rename(str(os.path.join(anat_dir, file)),
                                  str(os.path.join(workspace_dir, subject, 'ANATOMICAL', 'MP2RAGE_T1MAPS_.nii')))
                    elif 'mp2rage_p3_602B_UNI_Images_2.98' in file:
                        os.rename(str(os.path.join(anat_dir, file)),
                                  str(os.path.join(workspace_dir, subject, 'ANATOMICAL', 'MP2RAGE_UNI_.nii')))
