...
def get_atak_nuclei(population, workspace_dir, popname):
    ...
    for subject in population:
        ...
        atak_dir    = os.path.join(workspace_dir, subject, 'SEGMENTATION', 'ATAK')
        mkdir_path(atak_dir)
        os.chdir(atak_dir)
        os.system('cp %s ./transform1InverseWarp.nii.gz' % mni_invwarp)

        def transform_ATAK_2_NATIVE(mni_image, label_name, thr = 0.7):
            if not os.path.isfile('%sx.nii.gz'%label_name):
                print '.....Transforming %s to QSM SPACE' % label_name
                os.system('WarpImageMultiTransform 3 %s %s_wimt.nii.gz -R %s -i %s transform1InverseWarp.nii.gz'%(mni_image, label_name, anat, mni_affine))
