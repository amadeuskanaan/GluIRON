...
def make_coord_spheres(roi):
    ...
    for radius  in ['3mm']: # ,'2mm' , '3mm'
        ...
        spheres_path = mkdir_path(os.path.join(mask_path, 'AHBA_SPHERES_%s'%radius))
        ...
        for well in df.index:
            if not os.path.isfile('%s.nii.gz' % well):
                ...
                os.system('fslmaths %s -mul 0 -add 1 -roi  %s 1 %s 1 %s 1 0 1 %s.nii.gz -odt float'  %(mni_brain_05mm, X,Y,Z, well))

        roi_list = ' '.join(['%s.nii.gz -add '%well for well in df.index])[0:-5]
        print roi_list
        os.system('fslmaths %s -bin ../%s_spheres_%s.nii.gz' % (roi_list, roi,radius))
