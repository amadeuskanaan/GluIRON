...
def concat(population, workspace_dir, population_name, quant_name):
    ...
    for subject_id in population:
        ...
        Xi  = pd.read_csv(os.path.join(workspace_dir, subject, stats_dir_name, 'nucleus_stats_%s.csv'%quant_name))
        ...
        dict_all        = dict(  dict_dmg.items()
                               + dict_seg.items()
                               + dict_qc.items()
                               + dict_first.items()
                               + dict_atak.items()
                               # + dict_freesurfer.items()
                               # + dict_mrs.items()
                               )
...
concat(lemon_population, workspace_study_a, population_name='LEMON', quant_name='QSM_median')
